...
original_fluid_solver::original_fluid_solver(const fluid_sandbox_config& cfg) : fluid_solver(cfg)
{
...
void original_fluid_solver::solve()
{
    int N = config.n.x;
    
    vel_step(N, u_, v_, u0_, v0_, config.visc, config.dt);
    dens_step(N, x_, x0_, u_, v_, config.diff, config.dt);
    
    const auto pixel_count = get_pixel_count();
    memset(x0_, 0, pixel_count * sizeof(float));
    memset(u0_, 0, pixel_count * sizeof(float));
    memset(v0_, 0, pixel_count * sizeof(float));
}

void original_fluid_solver::update_velocity_texture(GLuint texture)
{
