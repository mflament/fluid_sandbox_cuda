...
{
protected:
    fluid_solver_config config_{};

public:
    explicit fluid_solver(const fluid_solver_config& cfg) : config_(cfg)
    {
    }

    [[nodiscard]] fluid_solver_config config() const { return config_; }

    virtual ~fluid_solver() = default;

    virtual const char* solver_name() = 0;
